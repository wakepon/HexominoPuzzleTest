...
例: 「透の加護」のセル
→ 既にブロックがあっても、そのセルに新しいブロックを重ねて配置できる
...
## 加護一覧（全5種：power, gold, chain, phase, heal）
...
| type | `phase` |
| 名前 | 透の加護 |
| 説明 | このセルにブロックを重ねて配置できる |
| 効果対象 | 配置ルール |

**効果**: この加護が刻まれたセルにすでにブロックがある場合でも、そのセルに重ねてブロックを配置できるようになる。重ねて配置した場合、既存のブロックは上書きされる（新しいブロックのblockSetIdに置き換わる）。

| Lv | 効果 |
|----|------|
| Lv1 | 重ね配置可能 |
| Lv2 | 重ね配置可能 + 重ねたブロックのブロック点+1 |
| Lv3 | 重ね配置可能 + 重ねたブロックのブロック点+2 |

**配置ルールへの影響**:

- CollisionService の配置判定で、`phase`加護のあるセルは「空き」として扱う
- 重ね配置時、既存ブロックの blockSetId は新しいブロックの blockSetId で上書きされる
- 既存ブロックのシールや加護情報は失われる（セルの加護は維持される）
- obstacleセルには効果なし（obstacleは加護を封印する）
...
パターン・シール・加護付与確率はミノサイズに関わらず一律である（加護のみ未実装）。それぞれ独立して判定される。

| 商品枠 | 対象ミノカテゴリ | パターン付与確率 | シール付与確率 | 加護付与確率 |
|--------|----------------|--------------|--------------|-------------|
| 小中枠 | モノミノ/ドミノ/トリミノ/テトロミノ | 40% | 25% | 低確率 |
| 中大枠 | テトロミノ/ペントミノ/ヘキソミノ | 40% | 25% | 低確率 |
...
2. 加護効果を計算 ← NEW
   ...
   - chain: 列点(B)に +Lv 加算
   - phase: 配置ルールに影響（スコア計算時には効果なし。Lv2+はブロック点ボーナス）
   - heal: 確率で手数+1（スコア計算外）
3. パターン効果を計算
4. シール効果を計算
5. レリック効果を計算
