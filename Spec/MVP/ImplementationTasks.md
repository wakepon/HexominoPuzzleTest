...
| # | タスク | 説明 | ステータス |
|---|--------|------|:----------:|
| 3.1 | PatternEffectHandler | パターン効果計算（enhanced, aura, moss, lucky, combo）の純粋関数を作成 | ✅ 完了 |
| 3.2 | LineService拡張 | `calculateScoreWithEffects()` でパターン効果を考慮したスコア計算 | ✅ 完了 |
| 3.3 | GameReducer拡張 | ライン消去時にパターン効果を反映、comboCount追加 | ✅ 完了 |
| 3.4 | スコア表示拡張 | ボーナス内訳表示（オプション - ScoreBreakdown型で取得可能） | ✅ 完了 |
| 3.5 | ビルド確認 | `npx tsc --noEmit` でエラーがないことを確認 | ✅ 完了 |
...
| # | タスク | 説明 | ステータス |
|---|--------|------|:----------:|
| 4.1 | SealEffectHandler | シール効果計算（gold, score, multi, stone）の純粋関数を作成 | ✅ 完了 |
| 4.2 | LineService拡張 | 石シール付きセルをライン消去対象から除外 | ✅ 完了 |
| 4.3 | LineService拡張 | `calculateScore()` でシール効果（multi, score）を反映 | ✅ 完了 |
| 4.4 | GameReducer拡張 | ゴールドシール消去時にゴールド加算 | ✅ 完了 |
| 4.5 | ビルド確認 | `npx tsc --noEmit` でエラーがないことを確認 | ✅ 完了 |
...
| # | タスク | 説明 | ステータス |
|---|--------|------|:----------:|
| 5.1 | PlayerState型定義 | `src/lib/game/Domain/Player/PlayerState.ts` を作成 | ✅ 完了 |
| 5.2 | GameState拡張 | `player: PlayerState` フィールドを追加 | ✅ 完了 |
| 5.3 | PlayerReducer作成 | レリック追加/削除のReducer | ✅ 完了 |
| 5.4 | ショップにレリック追加 | `RelicShopItem` を生成してショップに含める | ✅ 完了 |
| 5.5 | 購入ロジック | レリック購入時にPlayerStateに追加 | ✅ 完了 |
| 5.6 | relicPanelRenderer作成 | 画面上部に所持レリックアイコンを描画 | ✅ 完了 |
| 5.7 | shopRenderer拡張 | レリック商品の表示（アイコン、説明、レアリティ） | ✅ 完了 |
| 5.8 | ビルド確認 | `npx tsc --noEmit` でエラーがないことを確認 | ✅ 完了 |
...
| # | タスク | 説明 | ステータス |
|---|--------|------|:----------:|
| 6.1 | RelicEffectHandler | レリック効果計算の純粋関数を作成 | ✅ 完了 |
| 6.2 | ScoreCalculator統合 | パターン/シール/レリック効果を統合したスコア計算 | ✅ 完了 |
| 6.3 | GameReducer拡張 | レリック効果をスコア計算に反映 | ✅ 完了 |
| 6.4 | 効果発動表示 | レリック発動時のエフェクト描画 | ✅ 完了 |
| 6.5 | ビルド確認 | `npx tsc --noEmit` でエラーがないことを確認 | ✅ 完了 |
...
| # | タスク | 説明 | ステータス |
|---|--------|------|:----------:|
| 7.1 | RoundTypes拡張 | `RoundType`, `BossCondition`, `RoundInfo` を定義 | ✅ 完了 |
| 7.2 | RoundService拡張 | ラウンド情報計算、ボス条件抽選 | ✅ 完了 |
| 7.3 | GameState拡張 | `roundInfo: RoundInfo` に変更 | ✅ 完了 |
| 7.4 | ボス条件適用 | おじゃまブロック配置、省エネ（配置数減）、手札2枚 | ✅ 完了 |
| 7.5 | roundRenderer拡張 | ラウンドタイプ、ボス条件の表示 | ✅ 完了 |
| 7.6 | ビルド確認 | `npx tsc --noEmit` でエラーがないことを確認 | ✅ 完了 |
...
| # | タスク | 説明 | ステータス |
|---|--------|------|:----------:|
| 8.1 | ShopPriceCalculator | Piece/レリックの価格自動計算 | ✅ 完了 |
| 8.2 | ショップUI改善 | 価格表示、購入可否のスタイル | ✅ 完了 |
| 8.3 | セール機能 | 一部商品にセールフラグ | ✅ 完了 |
| 8.4 | ビルド確認 | `npx tsc --noEmit` でエラーがないことを確認 | ✅ 完了 |
...
| # | タスク | 説明 | ステータス |
|---|--------|------|:----------:|
| 9.1 | GameEvent型定義 | イベント型（PiecePlaced, LinesCompleted等）を定義 | ✅ 完了 |
| 9.2 | EventBus実装 | イベント登録・発火・ログ機能 | ✅ 完了 |
| 9.3 | EffectHandlers統合 | イベントハンドラとしてエフェクト計算を登録 | ✅ 完了 |
| 9.4 | useGameEvents hook | イベントバス連携hook | ✅ 完了 |
| 9.5 | ビルド確認 | `npx tsc --noEmit` でエラーがないことを確認 | ✅ 完了 |
...
| # | タスク | 説明 | ステータス |
|---|--------|------|:----------:|
| 10.1 | リセット処理 | ゲームオーバー時のPlayerState/ショップ/ラウンドリセット | ✅ 完了 |
| 10.2 | 永続化（オプション） | LocalStorageへの保存・復元 | ✅ 完了 |
| 10.3 | ビルド確認 | `npx tsc --noEmit` でエラーがないことを確認 | ✅ 完了 |
...
| 3: パターン効果計算 | 5 | 5 | 100% |
| 4: シール効果計算 | 5 | 5 | 100% |
| 5: レリックシステム基盤 | 8 | 8 | 100% |
| 6: レリック効果計算 | 5 | 5 | 100% |
| 7: ラウンドセット + ボス条件 | 6 | 6 | 100% |
| 8: ショップ価格計算 + UI改善 | 4 | 4 | 100% |
| 9: イベントシステム（オプション） | 5 | 5 | 100% |
| 10: ゲームリセット + 永続化 | 3 | 3 | 100% |
| **合計** | **58** | **58** | **100%** |
