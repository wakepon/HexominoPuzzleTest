...
| パターンID | 効果 | multiシール |
|-----------|------|------------|
| `enhanced` | ブロック点+2/個 | パターン効果2倍（+4/個） |
| `aura` | 隣接する別セットauraブロックがあるセルはブロック点+1（1セルあたり上限+1） | パターン効果2倍（+2） |
| `charge` | 基礎ブロック数から除外され、chargeValue合計をブロック点に加算 | パターン効果2倍（chargeValue×2） |
...
| パターンID | 効果 | multiシール |
|-----------|------|------------|
| `moss` | 端接触辺数だけ列点に加算（角で+2、辺で+1） | パターン効果2倍 |
...
| シールID | 影響先 | 効果 |
|---------|--------|------|
| `multi` | ブロック点(A) | ブロック点+1/個（2回カウント）。さらにパターン効果を2倍化（enhanced, aura, moss, charge, comboが2倍、luckyは2回抽選） |
...
| `nobi_takenoko` | 縦列のみ消去 | 列点×累積倍率（初期1.0、+0.5ずつ増加、横行消しで1.0にリセット） |
| `nobi_kani` | 横行のみ消去 | 列点×累積倍率（初期1.0、+0.5ずつ増加、縦列消しで1.0にリセット） |
| `rensha` | ライン消去時 | 列点×累積倍率（初期1.0、+1ずつ増加、消去なしで1.0にリセット） |
| `timing` | 残りハンド数が3で割り切れる、かつライン消去あり | 列点×3 |
...
| レリックID | 発動条件 | 効果 |
|-----------|---------|------|
| `size_bonus_1` ～ `size_bonus_6` | 対応サイズのピース（1～6ブロック）でライン消去 | 消去ブロック数（baseBlocks）をブロック点に加算 |
...
2. パターン効果計算:
   enhancedBonus = enhanced付きセル数 × 2（multi付きならパターン効果2倍で×4）
   auraBonus     = 隣接に別セットauraがある消去セル数（multi付きならパターン効果2倍で+2）
   chargeBonus   = charge付きセルのchargeValue合計（multi付きならパターン効果2倍でchargeValue×2）
   mossBonus     = moss付きセルの端接触辺数合計（multi付きならパターン効果2倍）
