...
6. 購入可能な場合:
   ...
   - アイテム種類に応じた処理:
     - `block`: ミノIDをデッキに追加
     - `pattern`: パターン付きブロックをデッキに追加
     - `seal`: シール付きブロックをデッキに追加
     - `relic`: レリックを所持リストに追加
...
1. `shopping` フェーズでのみ実行可能
2. ショップ状態を `null` に設定
3. `round_progress` フェーズに遷移

**タイミング:**
- ユーザーが「次へ」ボタンをクリックした時

### START_ROUND

**処理内容:**
1. `round_progress` フェーズでのみ実行可能
2. 次のラウンドの状態を作成:
   - ラウンド番号を+1
   - ボードをリセット
   ...
   - スコアを0にリセット
   - デッキをシャッフル（購入したミノを含む）
   - 配置回数をリセット（ボス条件「省エネ」の場合は減少）
   - 目標スコアを計算
   - ドロー枚数を決定（ボス条件「手札2枚」の場合は2）
   - デッキから新しいミノを引く
   - `playing` フェーズに遷移

**タイミング:**
- ユーザーがラウンド進行画面で「ラウンド開始」ボタンをクリックした時

### ADD_RELIC

**処理内容:**
1. レリックIDをレリック状態の所持リストに追加
