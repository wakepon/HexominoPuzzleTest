...
Balatroのジョーカー中心設計に倣い、**レリックを戦略の主軸**に据える。ピースは「スコアを出す媒体」、レリックは「どう稼ぐかの戦略」、護符は「デッキの質を調整する戦術」。
...
- **レリック**: 恒久的なパッシブ効果を持つアイテム。所持上限5枠、売買可能
- **デッキビルディング**: ラウンドクリア後にショップで新しいブロックをデッキに追加
- **ラウンドセット**: 雑魚→エリート→ボスの3ラウンドを1セットとして進行
- **パターン・シール**: ブロックに特殊効果を付与
- **護符**: デッキの質を調整する消費アイテム（ストック可能、最大2個）
- **ゴールドシステム**: ラウンドクリア報酬 + 利息でショップアイテムを購入
...
> ※現状は初期デッキ9枚（モノミノ1 + ドミノ2 + トロミノ6）。6枚化は未実装。
...
- **利息**: ラウンドクリア時に所持ゴールド5Gごとに+1G（上限+5G/ラウンド）
...
- 詳細は[レリックシステム](./relic-system.md)の護符セクションを参照
